...
def get_platform_colors(platform):
    ...
    match platform:
        case 'Spotify':
            color_data['platform_logo'] = "https://upload.wikimedia.org/wikipedia/commons/thumb/1/19/Spotify_logo_without_text.svg/2048px-Spotify_logo_without_text.svg.png"
        ...
        case 'Youtube':
            color_data['platform_logo'] = "https://1000logos.net/wp-content/uploads/2017/05/Red-YouTube-logo.png"
        ...
        case _:
            ...
            color_data['platform_logo'] = "https://upload.wikimedia.org/wikipedia/en/thumb/9/9e/Flag_of_Japan.svg/1280px-Flag_of_Japan.svg.png"
...
def queue_song(queue_size=0, song_title="Song Title", song_artist="", song_url="", duration_string="", requester="You", thumbnail="", platform="Spotify"):
    ...
    embed = discord.Embed(title=f"{song_title}", description= (f"by {song_artist}  |  " if song_artist else "") + f"`{duration_string}`  |   [{emoji} Song Link]({song_url})", color=color)
...
def now_playing(song_title="Song Title", song_artist="", song_url="", duration_string="", platform="Spotify", requester="", thumbnail=""):
    ...
    # Build embed
    embed = discord.Embed(title=f"{notes} {song_title} {notes}", description= (f"by {song_artist}  |  " if song_artist else "") + f"`{duration_string}`  |  [{emoji} Song Link]({song_url})" + (f"\n\nRequested by: @{requester}" if requester else ""), color=color)
