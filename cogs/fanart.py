...
class FanArt(commands.Cog):
    ...
    def _decodeHTMLContent(self, content):
        ...
        endIndex = (
            content.find(paranthesis)
            if content.find(paranthesis) < content.find(dash)
            else content.find(dash)
        )
    ...
    async def _getPictures(self, ctx, tag, tries=0):
        headers = {"User-Agent": "Gamer Bot - B Box9688"}

        try:
            r = requests.get(
                f"https://www.zerochan.net/{tag}?p=1&l=25&s=fav&json",
                headers=headers,
                timeout=5,
            )
            r.raise_for_status()
    ...
    async def generate(self, ctx, *, tag: str):
        ...
        if not items:
            await ctx.send(
                f"No Pictures Found! <:gi_hutao_notlikethis:1187215478083563662>\n\nTry another tag!"
            )
        else:
